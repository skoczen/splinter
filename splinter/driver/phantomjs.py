...
class PhantomTestBrowser(DriverAPI):

    driver_name = "Phantom.testbrowser"
    ...
    def find_option_by_value(self, value):
        ...
        return ElementList([PhantomTestBrowserOptionElement(control, self)], find_by="value", query=value)

    def find_option_by_text(self, text):
        html = lxml.html.fromstring(self.html)
        element = html.xpath('//option[normalize-space(text())="%s"]' % text)[0]
        control = self._browser.getControl(element.text)
        return ElementList([PhantomTestBrowserOptionElement(control, self)], find_by="text", query=text)
    ...
    def find_by_xpath(self, xpath, original_find=None, original_selector=None):
        ...
        return ElementList([PhantomTestBrowserElement(element, self) for element in elements], find_by=find_by, query=query)
    ...
    def find_by_name(self, name):
        ...
        while True:
            try:
                ...
                break
        return ElementList([PhantomTestBrowserControlElement(element, self) for element in elements], find_by="name", query=name)
    ...
    def _find_links_by_xpath(self, xpath):
        ...
        return ElementList([PhantomTestBrowserLinkElement(link, self) for link in links], find_by="xpath", query=xpath)
...
class PhantomTestBrowserElement(ElementAPI):
...
class PhantomTestBrowserLinkElement(PhantomTestBrowserElement):

    def __init__(self, element, parent):
        super(PhantomTestBrowserLinkElement, self).__init__(element, parent)
        self._browser = parent._browser

    def __getitem__(self, attr):
        return super(PhantomTestBrowserLinkElement, self).__getitem__(attr)

    def click(self):
        return self._browser.open(self["href"])


class PhantomTestBrowserControlElement(ElementAPI):
...
class PhantomTestBrowserOptionElement(ElementAPI):
